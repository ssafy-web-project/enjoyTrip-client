...
const mainStore = {
  ...
  actions: {
    getMonthInfo({ commit }) {
      http
        .get(`http://localhost:8080/api/v1/spot/detail/25372`)
        .then(({ data }) => {
    ...
    getThemaInfo({ commit }, theme) {
      http
        .get(`http://localhost:8080/api/v1/spot/top/${theme}`)
        .then(({ data }) => {
    ...
    getPopularInfo({ commit }) {
      http
        .get("http://localhost:8080/api/v1/board/top")
        .then(({ data }) => {
    ...
    async getPopularDetailInfo({ commit }, id) {
      await http
        .get(`http://localhost:8080/api/v1/board/top/${id}`)
        .then(({ data }) => {
