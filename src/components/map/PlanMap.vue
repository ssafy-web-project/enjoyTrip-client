...
export default {
  ...
  methods: {
    ...
    displayInfowindow(marker, spot) {
      ...
      if (spot.firstImage)
        content += `<img src="${spot.firstImage}" alt="${spot.title}" width = "50px" height = "50px"/>`;
      else
        content += ` <img src="${require("@/assets/noimg.gif")}" alt="${
          spot.title
    ...
    addSchedule(spot) {
      ...
      for (let i = 0; i < this.sSchedules.length; i++) {
        if (this.sSchedules[i].contentId == spot.contentId) {
          flag = false;
        }
      }
    ...
    upSchedule(contentId) {
      for (var i = 1; i < this.sSchedules.length; i++) {
        if (this.sSchedules[i].contentId == contentId) {
