      ...
      <div>
          ...
          <div class="d-flex justify-content-end">
            ...
            <div>
              <button
                class="link"
                v-if="
                  currentUser.id === commentToComment.write_comment_user &&
                  isAliveComment
                "
                @click="openCommentForm2"
                style="margin-right: 6px; height: 30px"
              >
                수정
              </button>
              ...
              <div v-if="openToggle">
                <form v-if="value === 2" @submit.prevent="updateComment">
                  ...
                  <input
                    ...
                    required
                  />
                  <button class="link" style="width: 35px; height: 30px">
                    등록
                  </button>
                </form>
              </div>
        ...
        <div class="d-flex justify-content-between">
          <div class="speech" data-tooltip="Like">
            <div @click="pressLike" class="heart">
              💟{{ commentToComment.like_comment_users.length }}
            </div>
          </div>
          <div style="font-size: 12px; color: grey">
            <div class="d-flex justify-content-end">
              created : {{ commentToComment.created_at }}
            </div>
            <div class="d-flex justify-content-end">
              updated : {{ commentToComment.updated_at }}
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  computed: {
    ...
    hasComment() {
      return (
        this.commentToComment.commented &&
        this.commentToComment.commented.length > 0
      );
    },
    commentMargin() {
      return {
  ...
  methods: {
    ...
    writeComment() {
      ...
      this.createCommentToComment({
        reviewId: this.commentToComment.commented_review,
        content: this.content,
        superCommentId: this.commentToComment.id,
      });
      this.content = "";
    },
    updateComment() {
      this.updateCommentToComment({
        reviewId: this.commentToComment.commented_review,
        content: this.awContent,
        superCommentId: this.commentToComment.id,
      });
    },
    deleteComment() {
    ...
    pressLike() {
      this.likeComment({
        reviewId: this.commentToComment.commented_review,
        currentUserId: this.currentUser.pk,
        commentId: this.commentToComment.id,
      });
    },
  },
};
