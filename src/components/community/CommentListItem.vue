  ...
  <div class="comment-list-item">
    <div>
      <div class="d-flex justify-content-between" style="margin-bottom: 2%">
        <div style="margin-bottom: 10px; font-size: 20px" v-if="!isEditing">
          {{ payload.content }}
        </div>
        <!-- 댓글 출력 -->
        <div v-if="isAliveComment" class="d-flex justify-content-end">
          ...
          <div
            class="d-flex"
            v-if="currentUser.id === comment.write_comment_user && !isEditing"
          >
            <button
              class="link"
              style="width: 35px; height: 30px; margin-right: 10px"
      ...
      <div class="d-flex justify-content-between">
        <div class="speech" data-tooltip="Like">
          <div class="heart" @click="pressLike">
            💟{{ this.comment.like_comment_users.length }}
          </div>
          <!-- 좋아요 누르기 -->
        </div>
  ...
  methods: {
    ...
    writeComment() {
      this.createCommentToComment({
        ...
        superCommentId: this.comment.id,
      });
      this.writtenContent = "";
      alert("댓글이 등록되었습니다.");
      this.fetchComments(this.comment.commented_review);
