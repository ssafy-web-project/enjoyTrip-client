<template>
  <div style="display: flex; height: 84vh">
    <!-- <div class="section-header">
        <h2>여행 계획 짜기</h2>
        <p>
          관광지 검색 후 <strong>더블 클릭</strong>해서 여행 계획에 추가해
          보세요!
        </p>
      </div> -->
    <div id="menu_wrap" class="bg_white">
      ...
      <ul id="placesList">
        <plan-search-item
          ...
          @dblclick.native="addSchedule(spot)"></plan-search-item>
      </ul>
    </div>
    <div class="map_wrap">
    ...
    <div>
      <plan-write
        ...
        v-on:deleteSchedule="deleteSchedule"></plan-write>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    upSchedule(contentId) {
      this.$refs.map.upSchedule(contentId);
    },
    downSchedule(contentId) {
      this.$refs.map.downSchedule(contentId);
    },
    deleteSchedule(contentId) {
      this.$refs.map.deleteSchedule(contentId);
    },
  },
  created() {
    this.CLEAR_SEARCHSPOT_LIST();
